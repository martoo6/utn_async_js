...
locations.forEach(location => {
  request(searchLocationUrl(location), onSearchLocation);
});

const onSearchLocation = (err, res) => {
  locationIds.push(res.body[0].woeid);
  if (locationIds.length == locations.length) {
    //Let's get the humidity for each location
    const humidites = [];
    locationIds.forEach(locationId =>
      request(getLocationUrl(locationId), onGetLocation)
    );
  }
};

const onGetLocation = (err, res) => {
  let localAvg = 0;
